...
If `twCan` doesn't work for you, you can create your own custom merge function with [`extendTailwindMerge`](#extendtailwindmerge).
...
It is used internally within `twCan` and a direct subset of [`clsx`](https://www.npmjs.com/package/clsx). If you use `clsx` or [`classnames`](https://www.npmjs.com/package/classnames) to apply Tailwind classes conditionally and don't need support for object arguments, you can use `twJoin` instead, it is a little faster and will save you a few hundred bytes in bundle size.
...
const customtwCan = extendTailwindMerge({
    ...
    conflictingClassGroups: {
        ...
        // In this case `twCan('qux-auto foo') → 'foo'`
        foo: ['bar'],
    },
    ...
    conflictingClassGroupModifiers: {
        ...
        //     ↓ …classes from groups with these IDs
        // In this case `twCan('qux-auto baz-sm/1000') → 'baz-sm/1000'`
        baz: ['bar'],
    },
})
```
...
```ts
const customtwCan = extendTailwindMerge({ … }, withSomePlugin)
```
...
//   `customtwCan` gets called the first time.
...
```ts
const customtwCan = createTailwindMerge(getDefaultConfig, withSomePlugin, (config) => ({
...
```ts
const customtwCan = createTailwindMerge(getDefaultConfig, (config) =>
    mergeConfigs(config, {
        classGroups: {
