...
You'll be able to do this by creating a custom `twCan` functon with [`extendTailwindMerge`](./api-reference.md#extendtailwindmerge).
...
Note that by using `extendTailwindMerge` we're only adding our custom classes to the existing ones in the config, so `twCan('shadow-200 shadow-lg')` will return the string `shadow-lg`. In most cases that's fine because you won't use that class in your project.

If you expect classes like `shadow-lg` to be input in `twCan` and don't want the class to cause incorrect merges, you can explicitly override the class group with [`createTailwindMerge`](./api-reference.md#createtailwindmerge), removing the default classes.
...
function ButtonPrimary(props) {
    return <button {...props} className={twCan(BTN_PRIMARY_CLASSNAMES, props.className)} />
}
```

## Modifying inputs and output of `twCan`

> How do I make `twCan` accept the same argument types as clsx/classnames?

You can wrap `twCan` in another function which can modify the inputs and/or output.

```js
function customtwCan(...inputs) {
    const modifiedInputs = modifyInputs(inputs)
    return twCan(modifiedInputs)
}
```
